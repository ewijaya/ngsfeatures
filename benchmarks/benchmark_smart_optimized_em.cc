// =====================================================================================
// Benchmark for Smart Optimized EM Algorithm Performance
// Uses adaptive parallelization based on data size
...
// Threshold for when to use parallelization (empirically determined)
...
// ============================================================================
// SMART OPTIMIZED IMPLEMENTATIONS (adaptive parallelization)
// ============================================================================

double computeLogLik_smart(const std::vector<double>& m, const std::vector<double>& p,
                           const double& lmbd) {
    double Result = 0.0;

    if (p.size() >= PARALLEL_THRESHOLD) {
...
std::vector<double> divideVecWithScalar_smart(const std::vector<double>& Vec, const double& val) {
...
std::vector<double> divideVecWithVecCorsp_smart(const std::vector<double>& Vec1,
                                                const std::vector<double>& Vec2) {
    std::vector<double> Result(Vec1.size());

    if (Vec1.size() >= PARALLEL_THRESHOLD) {
#pragma omp parallel for schedule(static)
...
std::vector<double> multiplyVecWithVecCorsp_smart(const std::vector<double>& Vec1,
                                                  const std::vector<double>& Vec2) {
    std::vector<double> Result(Vec1.size());
...
// Sparse matrix-vector product with smart parallelization
// For large datasets, use chunked approach to reduce atomic contention
std::vector<double> sparseM_vec_prod_smart(const std::vector<double>& p,
                                           const std::vector<int>& rowId,
                                           const std::vector<int>& colId,
                                           const std::vector<double>& realVal) {
    std::vector<double> Result(p.size(), 0.0);

    if (rowId.size() >= PARALLEL_THRESHOLD) {
        // Parallel version with reduced atomic contention
#pragma omp parallel
        {
            std::vector<double> local_result(p.size(), 0.0);

#pragma omp for nowait schedule(static)
            for (size_t i = 0; i < rowId.size(); i++) {
                local_result[rowId[i]] += realVal[i] * p[colId[i]];
            }
...
void benchmark_em_iteration(size_t num_sequences, size_t num_iterations, int version) {
    ...
    for (size_t iter = 0; iter < num_iterations; ++iter) {
        ...
        } else {  // Smart optimized
            theP = divideVecWithScalar_smart(theM, lambda);
            sparseM_prod_P = sparseM_vec_prod_smart(theP, IA, JA, RA);
            rSums = divideVecWithVecCorsp_smart(rawCount, sparseM_prod_P);
            tsparseM_prod_rSums = sparseM_vec_prod_smart(rSums, JA, IA, RA);
            theM = multiplyVecWithVecCorsp_smart(theP, tsparseM_prod_rSums);
            logLik = computeLogLik_smart(theM, theP, lambda);
        }
    }

    double elapsed = timer.elapsed_ms();
    double iter_per_sec = (num_iterations * 1000.0) / elapsed;
    double speedup = (version == 0) ? 1.0 : 0.0;  // Will be calculated by caller

    const char* label = (version == 0) ? "Original " : "Optimized";
    std::cout << "  " << label << ": " << elapsed << " ms (" << iter_per_sec << " iter/sec)";
    if (version == 0) {
        std::cout << std::endl;
    }
}
...
int main() {
    ...
    std::cout << "Parallel threshold: " << PARALLEL_THRESHOLD << " elements" << std::endl;
    std::cout << "========================================" << std::endl;
#else
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "EM Algorithm Smart Optimization Benchmark" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
